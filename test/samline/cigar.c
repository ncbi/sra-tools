...
int op_count( const cigar_opt * cigar )
{
	int res = 0;
	while( cigar != NULL )
	{
...
char * produce_read( const cigar_opt * cigar, const char * ref_bases, const char * ins_bases )
{
	...
	if ( cigar != NULL )
	{
		int readlen = calc_readlen( cigar );
		if ( readlen > 0 )
		{
			int needed_ref_bases = calc_reflen( cigar );
			int available_ref_bases = ref_bases != NULL ? strlen( ref_bases ) : 0;
			if ( available_ref_bases >= needed_ref_bases )
			{
				int needed_ins_bases = calc_inslen( cigar );
				int available_ins_bases = ins_bases != NULL ? strlen( ins_bases ) : 0;	
				if ( available_ins_bases >= needed_ins_bases )
				{
