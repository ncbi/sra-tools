...
rc_t context_destroy( p_context ctx ) {
    ...
    if ( NULL != ctx -> src_schema_list ) {
        VNamelistRelease( ctx -> src_schema_list );
        ctx -> src_schema_list = NULL;
    }
    if ( NULL != ctx -> table ) {
        free( ( void* )ctx -> table );
...
uint32_t context_schema_count( p_context ctx ) {
    ...
    if ( NULL != ctx ) {
        if ( 0 != ctx -> src_schema_list ) {
            uint32_t count;
            if ( 0 == VNameListCount( ctx -> src_schema_list, &count ) ) {
                res = count;
            }
        }
    }
...
static void context_evaluate_options( const Args *my_args, p_context ctx ) {
    if ( ( NULL != my_args ) && ( NULL != ctx ) ) {
        ...
        nlt_make_VNamelist_from_string( &( ctx -> src_schema_list ), 
