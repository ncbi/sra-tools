...
function test_whole_spot_fastq {
    ...
    time "$TOOL" "$1" --include-technical --concatenate-reads -pf "$THREADS" -o "$1.faster.fastq"
...
function test_whole_spot_fasta {
    ...
    time "$TOOL" "$1" --include-technical --concatenate-reads --fasta -pf "$THREADS" -o "$1.faster.fasta"
...
function test_split_spot_fastq {
    ...
    time "$TOOL" "$1" --split-spot -pf "$THREADS" -o "$1.faster.fastq"
    produce_md5_and_compare "$1.faster.fastq" "$MD5REFERENCE"
}
...
function test_split_spot_fasta {
    ...
    time "$TOOL" "$1" --split-spot --fasta -pf "$THREADS" -o "$1.faster.fasta"
    produce_md5_and_compare "$1.faster.fasta" "$MD5REFERENCE"
}
...
function test_split_files_fastq {
    ...
    time "$TOOL" "$1" --split-files -pf "$THREADS" -o $1.faster.fastq
    for num in {1..4}
    do
...
function test_split_files_fasta {
    ...
    time "$TOOL" "$1" --split-files --fasta -pf "$THREADS" -o $1.faster.fasta
    for num in {1..4}
    do
...
function test_split_3_fastq {
    ...
    time "$TOOL" "$1" --split-3 -pf "$THREADS" -o $1.faster.fastq
    for num in {1..4}
    do
        if [ -f "$1.faster_$num.fastq" ]; then
            MD5REFERENCE="./ref_md5/$1.split_3.fastq_$num.reference.md5"
...
function test_split_3_fasta {
    ...
    time "$TOOL" "$1" --split-3 --fasta -pf "$THREADS" -o $1.faster.fasta
    for num in {1..4}
    do
        if [ -f "$1.faster_$num.fasta" ]; then
            MD5REFERENCE="./ref_md5/$1.split_3.fasta_$num.reference.md5"
...
function test_unsorted_fasta {
    ...
    time "$TOOL" "$1" --fasta-unsorted -pf "$THREADS" -o $1.faster.fasta --seq-defline '>$ac.$si $sn length=$rl'
...
function test_unsorted_fasta_parts {
    ...
    time "$TOOL" "$1" --fasta-unsorted -pf "$THREADS" -o $1.only_aligned --only-aligned --seq-defline '>$ac.$si $sn length=$rl'
    time "$TOOL" "$1" --fasta-unsorted -pf "$THREADS" -o $1.only_unaligned --only-unaligned --seq-defline '>$ac.$si $sn length=$rl'
