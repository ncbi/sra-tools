...
rc_t helper_resolve_accession( const KDirectory *my_dir, char ** path ) {
    ...
    if ( NULL != strchr ( *path, '/' ) ) { return 0; }
    rc = SRAPathMake( &my_sra_path, my_dir );
    if ( rc != 0 ) {
        if ( rcNotFound != GetRCState ( rc ) || rcDylib != GetRCTarget ( rc ) ) {
    ...
    if ( 0 == rc ) {
        ...
        if ( 0 == rc ) {
            ...
            if ( 0 == rc ) {
                ...
                while( idx < count && 0 == rc && false == *flag ) {
                    const char * name;
...
bool helper_detect_src_md5( const VTable * src_tab ) {
    ...
    if ( 0 == rc ) {
        ...
        if ( 0 == rc ) {
            ...
            if ( ( kptFile == pt )||( pt == ( kptFile | kptAlias ) ) ) {
                res = true;
            }
