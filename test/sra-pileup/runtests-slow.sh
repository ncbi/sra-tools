...
echo check_skiplist:
output=$(NCBI_SETTINGS=/ ${python_bin} check_skiplist.py ${bin_dir}/sra-pileup)
res=$?
if [ "$res" != "0" ];
...
ACC=SRR3332402
output=$(NCBI_SETTINGS=/ ${python_bin} test_diff_fastq_dump_vs_sam_dump.py -a ${ACC} -f ${bin_dir}/fastq-dump -m ${bin_dir}/sam-dump)
res=$?
if [ "$res" != "0" ];
...
output=$(NCBI_SETTINGS=/ ${python_bin} test_all_sam_dump_has_spotgroup.py -a ${ACC} -m ${bin_dir}/sam-dump)
res=$?
if [ "$res" != "0" ];
