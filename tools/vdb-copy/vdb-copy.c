...
/* ----------------------------------------------------------------------------------- */
static rc_t vdb_copy_cell( const VCursor * src_cursor,
                           VCursor * dst_cursor,
                           const p_col_def col,
                           uint64_t row_id ) {
    rc_t rc;
...
static rc_t vdb_copy_row_loop( const p_context ctx,
    ...
    while ( 0 == rc && num_gen_iterator_next( iter, &row_id, &rc ) ) {
        if ( 0 == rc ) rc = Quitting();    /* to be able to cancel the loop by signal */
        if ( 0 == rc ) {
...
static const char * REF_EX   = "REFERENCE:PRESERVE_QUAL,REFERENCE:READ";
