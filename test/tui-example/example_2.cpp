...
class ex2_view : public Dlg
{
    public :
        ex2_view( const char * a_caption ) : Dlg()
        {
            SetCaption( a_caption );
            Tui_Rect r;
            GetRect( r );
            set_status_line( r, "a status line" );
        }
        
        void set_status_line( Tui_Rect r, const char * status )
        {
            r.change( 1, r.get_h() - 1, -2, 1 );
            AddLabel( STATUS_ID, r, status );
        }
};
