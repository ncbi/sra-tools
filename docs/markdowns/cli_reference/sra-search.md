...
| `-a`\|`--algorithm <alg>` | Search algorithm, one of: FgrepStandard (default) FgrepBoyerMoore FgrepAho AgrepDP AgrepWuManber AgrepMyers AgrepMyersUnltd NucStrstr SmithWaterman |
| `-e`\|`--expression <expr>` | Query is an expression (currently only supported for NucStrstr) |
| `-S`\|`--score <number>` | Minimum match score (0..100), default 100 (perfect match); supported for all variants of Agrep and SmithWaterman. |
