...
echo Testing sra-stat from ${bin_dir}
...
rm -rf actual
mkdir -p actual
NCBI_SETTINGS=/ ${bin_dir}/sra-stat -x SRR600096 > actual/SRR600096
...
NCBI_SETTINGS=/ ${bin_dir}/sra-stat -x SRR618333 > actual/SRR618333
...
NCBI_SETTINGS=/ ${bin_dir}/sra-stat -x SRR413283 > actual/SRR413283
...
NCBI_SETTINGS=/ ${bin_dir}/sra-stat -x SRR619505 | perl -w strip-path-sdlr.pl > actual/SRR619505
...
NCBI_SETTINGS=/ ${bin_dir}/sra-stat -x SRR1985136 | perl -w strip-path.pl > actual/SRR1985136
...
if perl check-run-perm.pl ${bin_dir}/srapath ${R} > /dev/null 2>&1 ; \
 then                      ${bin_dir}/sra-stat -x ${R} > /dev/null ; fi
res=$?
if [ "$res" != "0" ];
