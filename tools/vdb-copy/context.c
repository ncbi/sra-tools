...
rc_t context_destroy( p_context ctx )
{
    ...
    if ( NULL != ctx -> src_path )
    {
        free( ( void* )( ctx -> src_path ) );
        ctx -> src_path = NULL;
    }
    if ( NULL != ctx -> dst_path )
    {
        free( ( void* )( ctx -> dst_path ) );
        ctx -> dst_path = NULL;
    }
    if ( NULL != ctx -> kfg_path )
    {
        free( ( void* )( ctx -> kfg_path ) );
        ctx -> kfg_path = NULL;
    }
    if ( NULL != ctx -> src_schema_list )
    {
    ...
    if ( NULL != ctx -> dst_schema_tabname )
    {
        free( ( void* )( ctx -> dst_schema_tabname ) );
        ctx -> dst_schema_tabname = NULL;
    }
    if ( NULL != ctx -> legacy_schema_file )
    {
        free( ( void* )( ctx -> legacy_schema_file ) );
        ctx -> legacy_schema_file = NULL;
    }
    if ( NULL != ctx -> legacy_dont_copy )
    {
...
static rc_t context_set_md5_mode( p_context ctx, const char *src )
{
    ...
    switch( src[ 0 ] )
    {
        ...
        default : ctx -> md5_mode = MD5_MODE_OFF; break;
    }
    return 0;
}
...
static rc_t context_set_blob_checksum( p_context ctx, const char *src )
{
    ...
    switch( src[ 0 ] )
    {
        ...
        default : ctx -> blob_checksum = BLOB_CHECKSUM_OFF; break;
    }
    return 0;
}
...
static bool context_check_if_usage_necessary( p_context ctx )
{
    ...
    if ( ( NULL == ctx -> src_path ) || ( NULL == ctx -> dst_path ) )
    {
    ...
    for ( idx = 0; idx < count && 0 == rc; ++idx )
    {
        ...
        if ( 0 == rc )
        {
            switch( idx )
            {
                ...
                default : break;    /* arguments #2,3... are ignored */
            }
        }
    }
    return rc;
...
uint32_t context_schema_count( p_context ctx )
{
    ...
    if ( NULL != ctx )
    {
        if ( 0 != ctx -> src_schema_list )
        {
            uint32_t count;
            if ( 0 == KNamelistCount( ctx -> src_schema_list, &count ) )
            {
                res = count;
            }
        }
...
{
    if ( ( NULL == my_args ) || ( NULL == ctx ) ) return;
    ...
    ctx -> force_legacy  = context_get_bool_option( my_args, OPTION_LEGACY, false );
