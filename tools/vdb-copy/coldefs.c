...
rc_t col_defs_exclude_these_columns( col_defs* defs, const char * prefix, const char * column_names )
{
    ...
    if ( NULL == column_names ) /* it is OK if we have not column-names to exclude */
    {
        return 0;
    }
    if ( 0 == column_names[ 0 ] )
        ...
        if ( NULL != col )
        {
            if ( only_copy_columns )
            {
                if ( col -> to_copy )
                {
                    if ( with_casts )
                    {
                        rc = KOutMsg( "col[%u] = %s ('%s'->'%s')\n",
                            idx, col -> name, col -> src_cast, col -> dst_cast );
                    }
                    else
                    {
                        rc = KOutMsg( "col[%u] = %s\n", idx, col -> name );
