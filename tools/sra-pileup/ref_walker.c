...
static rc_t CC populate_data( void *obj, const PlacementRecord *placement,
    ...
    if ( rc == 0 ) {
        ...
        if ( rc == 0 ) { rec->reverse = *orientation; }
    }
...
static rc_t ref_walker_init( struct ref_walker * self )
{
    ...
    if ( rc == 0 ) { rc = VDBManagerMakeRead ( &self->vmgr, self->dir ); }
    if ( rc == 0 ) { rc = VDBManagerMakeSRASchema( self->vmgr, &self->vschema ); }
    if ( rc == 0 ) { rc = AlignMgrMakeRead ( &self->amgr ); }
    if ( rc == 0 ) { rc =  VFSManagerMake ( &self->vfs_mgr ); }
    if ( rc == 0 ) { rc = VNamelistMake ( &self->sources, 10 ); }
    ...
    if ( rc != 0 ) { ref_walker_release( self ); }
    return rc;
}
                ...
                if ( rc == 0 ) {
                    ...
                    if ( rc == 0 ) {
                        ...
                        if ( rc == 0 ) {
                            ...
                            if ( rc == 0 ) {
                                if ( rwd->ref_start == 0 ) { rwd->ref_start = 1;}
