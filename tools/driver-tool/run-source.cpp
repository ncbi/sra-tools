            ...
            for (auto & file : files) {
                ...
                if (vcache == files.end()) {
                    ...
                    for (auto &location : file.locations) {
                        ...
                        auto const &run_source = file.make_source(location, accession, response.localInfo2(accession, file.name, file.type));
                    ...
                    for (auto &location : file.locations) {
                        ...
                        if (best != vcache->locations.end()) {
                            auto const &cache_source = file.make_source(*best, accession, response.localInfo2(accession, vcache->name, vcache->type));
