...
| `-V,--version` | Display program version information and exit --print-deflines |
| `--output TEXT` | Output archive path |
| `--platform TEXT` | Optional platform |
| `--readTypes TEXT` | file read types < B\|T(s) > |
| `--useAndDiscardNames` | Excludes: --name-column Discard file names |
| `--allowEarlyFileEnd` | Complete load at early end of one of the files --sa,--spot-assembly |
| `--help_errors,--help-errors` | Print error codes and descriptions |
| `--name-column` | TEXT:{NONE,NAME,RAW_NAME}=NAME Excludes: --useAndDiscardNames Database name for NAME column |
...
| `--threads` | UINT:INT in [1 - 256]=24 Max number of threads to use (set optional value to indicate the number of threads to use |
...
| `--max-err-count` | UINT:UINT in [0 - 4,294,967,295]=100 Maximum number of errors allowed |
...
| `--log-level` | TEXT:{trace,debug,info,warning,error}=info Log level |
