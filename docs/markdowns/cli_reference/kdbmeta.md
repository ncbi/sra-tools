...
| `-X`\|`--output <value>` | Output type: one of (xml text):  whether to generate well-formed XML. Default: xml (well-formed) |
...
- `<obj>=VALUE`: a simple value assignment where value string is text, and binary values use hex escape codes
