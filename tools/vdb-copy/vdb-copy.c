...
static const char * legacy_usage[] = { "declares the source to be a legacy object", NULL };
...
OptDef MyOptions[] =
{
    ...
    { OPTION_LEGACY, ALIAS_LEGACY, NULL, legacy_usage, 1, false, false }
};
...
rc_t CC Usage ( const Args * args )
{
    ...
    HelpOptionLine ( ALIAS_LEGACY, OPTION_LEGACY, NULL, legacy_usage );
                                     ...
                                     bool *redact )
{
    ...
    case SRA_READ_FILTER_REJECT   : 
        if ( false == ctx -> ignore_reject ) *pass = false;
        break;

    case SRA_READ_FILTER_REDACTED : 
        if ( false == ctx -> ignore_redact ) *redact = true;
        break;
    }
    return rc;
}
                               ...
                               redact_vals * rvals )
{
    ...
    while ( 0 == rc && num_gen_iterator_next( iter, &row_id, &rc ) )
    {
        ...
        if ( 0 == rc )
        {
            ...
            if ( 0 == rc )
            {
                ...
                {
                    if ( 0 == num_gen_iterator_percent( iter, 2, &percent ) )
    ...
    if ( rcVDB == GetRCModule( rc ) && 
         rcNoTarg == GetRCTarget( rc ) && 
         rcReading == GetRCContext( rc ) &&
         rcId == GetRCObject( rc ) &&
         rcInvalid == GetRCState( rc ) )
    {
        rc = 0;
    }
                                     ...
                                     bool is_legacy )
{
    ...
    if ( is_legacy )
    {
        ...
        if ( NULL == *dst_schema )
        {
            rc = VDBManagerMakeSchema( vdb_mgr, dst_schema );
            if ( 0 != rc )
            {
                PLOGERR( klogInt,
                         (klogInt,
                         rc, "VDBManagerMakeSchema(dst) failed", "" ) );
            }
            /* this is a NOOP now!
    ...
    if ( 0 == rc )
    {
        rc = VDBManagerCreateTable( vdb_mgr, dst_table, *dst_schema, ctx -> dst_schema_tabname,
                                      ...
                                      bool is_legacy )
{
    ...
    rc = col_defs_mark_writable_columns( columns, dst_table );
                                         ...
                                         const char * src_schema_tabname )
{
    ...
    rc_t rc = get_table_platform( ctx, ctx->src_path, &src_platform, '_' );
...
static rc_t vdb_copy_find_out_what_columns_to_use( const VTable * src_table,
                                                   ...
                                                   const char * excluded )
{
    bool cols_requested = ( ( NULL != requested ) &&
                            ( 0 != nlt_strcmp( requested, "*" ) ) );
                                                     ...
                                                     matcher * type_matcher )
{
    ...
    col_defs_unmark_do_not_redact_columns( columns, config -> do_not_redact_columns );
}
                ...
                if ( 0 == rc )
                {
                    ...
                    if ( 0 == rc )
                    {
                        ...
                        if ( 0 == rc )
                        {
                            ...
                            if ( 0 == rc )
                            {
                                rc = col_defs_mark_writable_columns( columns, dst_tab );
                                    ...
                                    VDatabase * dst_db )
{
    ...
    if ( 0 != rc ) return 0; /* we return 0 here, because it is OK to not have sub-tables */
                              ...
                              VDatabase * dst_db )
{
    ...
    if ( 0 != rc ) return 0; /* we return 0 here, because it is OK to not have sub-db's */
    rc = KNamelistCount( names, &count );
        ...
        if ( 0 == rc )
        {
            ...
            if ( 0 == rc )
            {
                ...
                if ( 0 == rc )
                {
                    ...
                    if ( MD5_MODE_ON == ctx -> md5_mode ) cmode |= kcmMD5;
...
static rc_t vdb_copy_main( const p_context ctx )
{
    ...
    if ( 0 == rc )
    {
        ...
        rc = helper_make_config_mgr( &config_mgr, ctx -> kfg_path ); /* helper.c */
        DISP_RC( rc, "vdb_copy_main:helper_make_config_mgr() failed" );
        if ( 0 == rc )
        {
            VDBManager * vdb_mgr;

            helper_read_config_values( config_mgr, &( ctx -> config ) ); /* helper.c */
