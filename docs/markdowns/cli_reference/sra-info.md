...
| `-f`\|`--format <format>` | One of csv, xml, json, or tab. See Output Formats. |
...
| `xml` | xml-style |
| `json` | json-style |
| `tab` | tab-separated values on one line |

**Notes:** csv and tab formats can only be used with a single query --schema does not support csv and tab
