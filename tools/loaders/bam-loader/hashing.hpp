...
namespace hashing {
    ...
    static uint64_t fnv1a(const char* key, std::size_t key_size, size_t hash = 14695981039346656037u)
    {
    ...
    static uint64_t MurmurHash ( const void * key, int len, uint64_t seed = 0)
    {
...
#ifdef __APPLE__
    static inline unsigned char *SHA1(const unsigned char *data, size_t count, unsigned char *md_buf) {
        return ::CC_SHA1(data, count, md_buf);
    }
    static inline unsigned char *SHA224(const unsigned char *data, size_t count, unsigned char *md_buf) {
        return ::CC_SHA224(data, count, md_buf);
    }
    static inline unsigned char *SHA256(const unsigned char *data, size_t count, unsigned char *md_buf) {
        return ::CC_SHA256(data, count, md_buf);
    }
#else
    static inline unsigned char *SHA1(const unsigned char *data, size_t count, unsigned char *md_buf) {
        return ::SHA1(data, count, md_buf);
    }
    static inline unsigned char *SHA224(const unsigned char *data, size_t count, unsigned char *md_buf) {
        return ::SHA224(data, count, md_buf);
    }
    static inline unsigned char *SHA256(const unsigned char *data, size_t count, unsigned char *md_buf) {
        return ::SHA256(data, count, md_buf);
    }
#endif
}
