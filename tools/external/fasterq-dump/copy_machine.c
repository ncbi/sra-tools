...
static rc_t cm_destroy( copy_machine_t * self ) {
    uint32_t i;
...
static rc_t cm_push2q( KQueue * q, const void * item, uint32_t wait_time ) {
    rc_t rc;
    bool running = true;
    while ( running ) {
        struct timeout_t tm;
...
static rc_t cm_copy_this_file( copy_machine_t * self, const struct KFile * src ) {
    rc_t rc = 0;
    uint64_t src_pos = 0;
    size_t num_read = 1;
        ...
        if ( 0 != rc ) {
            ...
            if ( 0 == rc ) {
                ...
                if ( 0 != rc ) {
                    ...
                    src_pos += num_read;
                    rc = cm_push2q ( self -> to_write_q, block, self -> q_wait_time );
                } else {
                    /* we are done with this file, put the block back... */
                    rc = cm_push2q ( self -> empty_q, block, self -> q_wait_time );
                }
            }
...
static rc_t cm_run( copy_machine_t * self ) {
    uint32_t file_count;
        ...
        if ( 0 != rc ) {
            ...
            if ( 0 == rc ) {
                ...
                if ( 0 == rc ) {
                    ...
                    block -> available = 0;
                    rc = cm_push2q ( self -> empty_q, block, self -> q_wait_time ); /* above */
                } else {
...
rc_t cm_make_a_copy( KDirectory * dir,
    ...
    if ( NULL == dst || 0 == buf_size || NULL == sources ) {
        ...
        for ( i = 0; 0 == rc && i < N_COPY_MACHINE_BLOCKS; ++i ) {
            rc = cm_push2q ( cm . empty_q, &( cm . blocks[ i ] ), q_wait_time );
            if ( 0 != rc ) {
        ...
        if ( 0 == rc ) {
            rc = hlp_make_thread( &( cm . thread ), cm_writer_thread,
        ...
        /* collect the data on the current thread */
        if ( 0 == rc ) {
            rc = cm_run( &cm );
        }

        {
            rc_t rc2 = cm_destroy( &cm );
            rc = ( 0 == rc ) ? rc2 : rc;
        }
    }
    return rc;
