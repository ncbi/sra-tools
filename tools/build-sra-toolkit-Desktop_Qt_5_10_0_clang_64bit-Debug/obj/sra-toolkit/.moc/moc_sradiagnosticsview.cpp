/****************************************************************************
** Meta object code from reading C++ file 'sradiagnosticsview.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.10.0)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../../../sra-toolkit/diagnostics/sradiagnosticsview.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'sradiagnosticsview.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.10.0. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
QT_WARNING_PUSH
QT_WARNING_DISABLE_DEPRECATED
struct qt_meta_stringdata_SRADiagnosticsView_t {
    QByteArrayData data[8];
    char stringdata0[111];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_SRADiagnosticsView_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_SRADiagnosticsView_t qt_meta_stringdata_SRADiagnosticsView = {
    {
QT_MOC_LITERAL(0, 0, 18), // "SRADiagnosticsView"
QT_MOC_LITERAL(1, 19, 15), // "handle_callback"
QT_MOC_LITERAL(2, 35, 0), // ""
QT_MOC_LITERAL(3, 36, 16), // "DiagnosticsTest*"
QT_MOC_LITERAL(4, 53, 4), // "test"
QT_MOC_LITERAL(5, 58, 17), // "start_diagnostics"
QT_MOC_LITERAL(6, 76, 18), // "cancel_diagnostics"
QT_MOC_LITERAL(7, 95, 15) // "worker_finished"

    },
    "SRADiagnosticsView\0handle_callback\0\0"
    "DiagnosticsTest*\0test\0start_diagnostics\0"
    "cancel_diagnostics\0worker_finished"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_SRADiagnosticsView[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       4,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: name, argc, parameters, tag, flags
       1,    1,   34,    2, 0x0a /* Public */,
       5,    0,   37,    2, 0x08 /* Private */,
       6,    0,   38,    2, 0x08 /* Private */,
       7,    0,   39,    2, 0x08 /* Private */,

 // slots: parameters
    QMetaType::Void, 0x80000000 | 3,    4,
    QMetaType::Void,
    QMetaType::Void,
    QMetaType::Void,

       0        // eod
};

void SRADiagnosticsView::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        SRADiagnosticsView *_t = static_cast<SRADiagnosticsView *>(_o);
        Q_UNUSED(_t)
        switch (_id) {
        case 0: _t->handle_callback((*reinterpret_cast< DiagnosticsTest*(*)>(_a[1]))); break;
        case 1: _t->start_diagnostics(); break;
        case 2: _t->cancel_diagnostics(); break;
        case 3: _t->worker_finished(); break;
        default: ;
        }
    }
}

const QMetaObject SRADiagnosticsView::staticMetaObject = {
    { &QWidget::staticMetaObject, qt_meta_stringdata_SRADiagnosticsView.data,
      qt_meta_data_SRADiagnosticsView,  qt_static_metacall, nullptr, nullptr}
};


const QMetaObject *SRADiagnosticsView::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *SRADiagnosticsView::qt_metacast(const char *_clname)
{
    if (!_clname) return nullptr;
    if (!strcmp(_clname, qt_meta_stringdata_SRADiagnosticsView.stringdata0))
        return static_cast<void*>(this);
    return QWidget::qt_metacast(_clname);
}

int SRADiagnosticsView::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QWidget::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 4)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 4;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 4)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 4;
    }
    return _id;
}
QT_WARNING_POP
QT_END_MOC_NAMESPACE
